...
{ p = i; minNeedTime = j[i].need_time; }            // 第二种情况
...
for(int i=1;i<=num;i++)     {
jobList.push_back(jobs[i]); }}bool cmp(job a,job b)    {
return a.reach_time<b.reach_time;}void RR(){
double sum1=0; //总的等待时间
double sum2=0; //总的周转时间
double sum3=0; //总的带权周转时间
...
if(time_unit<10){               time_unit=10;           }
_sleep(1);
}
}
printf("--------所有作业调度完毕------\n");
printf("平均等待时间: %.2f 平均周转时间: %.2f 平均带权周转时间: %.2f",sum1/num,sum2/num,sum3/num);
initial_jobs_again();}int findNextHPF(job j[MAX_NUM], int length,int time ) {
//优先值越低 表示优先权越高
